<template>
  <data-table
    ...
    :paginator="true"
    :value="tableData"
    class="p-datatable-users shadow-lg"
    data-key="id"
  ...
  >
    ...
    >
      <template #body="{ data }">
        ...
        <span
          v-if="String(key) === 'statusName'"
          :class="`${
            data.statusName === 'Gotovo' ? 'bg-success' : 'bg-warning'
          } p-2 rounded-md`"
        ...
        <img
          ...
          v-if="String(key) === 'photo' && data.photo"
          :src="`data:image/webp;base64,${data.photo}`"
        />

        <img
          class="h-16 object-cover"
          v-else-if="String(key) === 'photo' && data.photo === null"
    ...
    <column>
      <template #body="slotProps"
        ><div class="space-x-2 flex justify-end" v-if="slotProps.data">
          <edit-delete-button
            isEdit
            v-if="!hideEdit"
            @ud-method="$emit('edit-method', slotProps.data.id)"
          />
          <edit-delete-button
            v-if="!hideDelete"
            @ud-method="$emit('delete-method', slotProps.data)"
          />
        </div> </template
    ></column>
  </data-table>
...
export default defineComponent({
  ...
  methods: {
    formatData(key: string, data: any) {
      if (key === 'total') {
        return `${data.price * data.quantity} kn`;
      }
